...
- `PostgreSQL`를 이용해 주세요.
- 데이터 모델 간의 관계를 고려하여 `onDelete`를 설정해 주세요.
- 데이터베이스 시딩 코드를 작성해 주세요.
- 각 API에 적절한 **에러 처리**를 해 주세요.
- 각 API 응답에 적절한 **상태 코드**를 리턴하도록 해 주세요.
...
- `name`, `description`에 포함된 단어로 **검색**할 수 있습니다.
...
- `offset` 방식의 **페이지네이션** 기능을 포함해 주세요.
- **최신순(`recent`)으로 정렬할 수 있습니다.**
- `title`, `content`에 포함된 단어로 **검색**할 수 있습니다.
...
- `multer` 미들웨어를 사용하여 **이미지 업로드 API**를 구현해주세요.
- 업로드된 이미지는 **서버에 저장**하고, 해당 이미지의 **경로를 response 객체에 포함해 반환**합니다.

---

## **에러 처리**

- 모든 예외 상황을 처리할 수 있는 **에러 핸들러 미들웨어**를 구현합니다.
- **상황에 맞는 상태값을 반환해야 합니다.**
  - `500` → **서버 오류**
  - `400` 시리즈 → **사용자 입력 오류**
  - `404` → **리소스를 찾을 수 없음**
...
- 중복되는 라우트 경로(예: `/users`에 대한 `GET` 및 `POST` 요청)를 `app.route()`로 통합해 **중복을 제거**합니다.
- `express.Router()`를 활용하여 **중고마켓/자유게시판 관련 라우트를 별도의 모듈로 구분**합니다.
